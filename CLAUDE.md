...
- `App.tsx` renders `src/screens/WalkScreen.tsx`, the single screen: a status line, the hero fact card, start/stop walk plus Tune, and the list of facts heard earlier. Its **Random fact** button exists for testing at home: it runs one loaded place through the same `speakFact` path as a real proximity trigger (LLM rewrite if a key is set, then speech and a history card), first fetching real Wikipedia places near the current fix or, with no fix, near the CN Tower. Keep triggers and Random fact on that shared path so home tests exercise the real pipeline. **Demo mode** is the bigger sibling of Random fact: long-pressing that button cycles a simulated walk (off, 1x, 5x, 20x) that drives `src/facts/demoRoute.ts` positions through the normal trigger, Wikipedia, LLM and speech path, so facts fire on their own indoors. While a demo is armed that button reads `Demo x1` / `x5` / `x20` instead of `Random fact`, which is its only on-screen indicator; it still speaks a random fact on a normal press, and it resets on every launch. Its one visible affordance is that tapping the coordinates in the status line, while a demo is running, opens `src/screens/DemoMapSheet.tsx` to move the walk: tap the map, confirm, and the loop restarts around that pin with a forced Wikipedia refetch. The device cannot hand a tapped point back from its own map app (the URL launch is one-way), so the sheet hosts a native map in-app via `react-native-maps` with no `provider` prop, which means Apple Maps on iOS and Google Maps on Android. This is the only dependency demo mode added. That sheet is deliberately **not** a `Modal` and its map frame has no `overflow: 'hidden'`: on Android a `MapView` inside a `Modal`, or clipped by a rounded parent, renders a grey surface with the Google logo and no tiles. It is an absolutely positioned overlay in the normal view tree instead, so do not fold it back into the shared `BottomSheet`. Expo Go needs no Google Maps API key; a store build does, via the `react-native-maps` config plugin.
...
- `src/log.ts` is the only logging path (`log(tag, ...)` / `logError(tag, err)`). Output appears in the terminal running `expo start` while the phone is connected. Log key events (location, wikipedia, trigger, narrate, llm, settings, demo, map) and every caught error; never log the API key.
